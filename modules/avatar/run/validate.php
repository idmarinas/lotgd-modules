...
switch ($mode)
{
    case 'invalidate':
        ...
        if ('' == $who)
        {
            ...
            {
                $name = '%'.$search.'%';
                $sql = 'SELECT u.userid AS acctid,k.name as name, k.login as login FROM '.DB::prefix('module_userprefs').' AS u INNER JOIN '.DB::prefix('module_userprefs').' AS t RIGHT JOIN '.DB::prefix('accounts')." as k ON u.userid=t.userid AND k.acctid=u.userid WHERE u.modulename='avatar' AND u.setting='avatar' AND u.value!='' AND t.modulename='avatar' AND t.setting='validated' AND t.value='1' AND (k.name LIKE '$name' OR k.login LIKE '$name');";
                $result = DB::query($sql);
                if (DB::num_rows($result) > 100)
                {
    ...
    case 'validate':
        if (httpget('giveok'))
        {
            ...
            require_once './lib/systemmail.php';
            systemmail(httpget('user'), ['Your avatar has been validated!'], ['`^As of now, your avatar will be visible.`n`nRegards`n%s`^`n `&Staff', $session['user']['name']]);
        }
        else
        {
            output_notl('`n`n');
        ...
        $sql = 'SELECT a.login as login, u.userid as acctid ,a.name as name,a.emailaddress as email FROM '.DB::prefix('module_userprefs').' AS u INNER JOIN '.DB::prefix('module_userprefs').' AS t RIGHT JOIN '.DB::prefix('accounts')." AS a ON u.userid=t.userid AND a.acctid=u.userid WHERE u.modulename='avatar' AND u.setting='avatar' AND u.value!='' AND t.modulename='avatar' AND t.setting='validated' AND t.value!='1' $where ORDER BY rand(".e_rand().') LIMIT 1;';
        $result = DB::query($sql);
    ...
    default:
        ...
        if ($user)
        {
            ...
            {
                $ok = (0 == get_module_pref('validated', 'avatar', $user) ? 1 : 0);
            }
        }
        if (httpget('giveok') && $ok)
        {
            $sql = 'SELECT name FROM '.DB::prefix('accounts').'  WHERE acctid='.((int) $user);
            $result = DB::query($sql);
            ...
            require_once './lib/systemmail.php';
            systemmail($user, ['Your avatar has been validated!'], ['`^As of now, your avatar will be visible.`n`nRegards`n%s`^`n `&Staff', $session['user']['name']]);
        ...
        elseif (httpget('deny') && $ok)
        {
            ...
            require_once './lib/systemmail.php';
            systemmail($user, ['`xYour avatar has `$NOT`x been validated!'], ['`^Your picture is not within the local policies/rules. Please choose a new one and petition/mail the appropriate link to us. If you want to upload a new one and need the points back, please also do so.`n`nRegards`n%s`^`n `&Staff', $session['user']['name']]);
        ...
        $sql = 'SELECT count(u.userid) AS counter FROM '.DB::prefix('module_userprefs').' AS u INNER JOIN '.DB::prefix('module_userprefs')." AS t ON u.userid=t.userid AND u.modulename='avatar' AND u.setting='avatar' AND u.value!='' AND t.modulename='avatar' AND t.setting='validated' AND t.value!='1';";
        $result = DB::query($sql);
