...
function worldmapen_run_real()
{
    ...
    if ('destination' == $op)
    {
        ...
        addnav(['Enter %s', $cname], 'village.php');
    ...
    if (httpget('comscroll') || 0 === httpget('comscroll') || httpget('comment') || httpget('refresh'))
    {
        $chatoverride = 1;
    ...
    elseif ('move' == $op && ! $chatoverride)
    {
        ...
        if ('1' == get_module_setting('compasspoints') and 'northeast' == $direction)
        {
            $y++;
            $x++;
        }

        if ('1' == get_module_setting('compasspoints') and 'northwest' == $direction)
        {
            $y++;
            $x--;
        }
        ...
        if ('1' == get_module_setting('compasspoints') and 'southeast' == $direction)
        {
            $y--;
            $x++;
        }

        if ('1' == get_module_setting('compasspoints') and 'southwest' == $direction)
        {
            $y--;
            $x--;
        }
        ...
        if (e_rand(0, 100) < $encounter && '1' != $su && 1 == $proceed && ! $chatoverride)
        {
            // They've hit a monster!
            if (0 != module_events(get_module_setting('randevent'), get_module_setting('wmspecialchance'), 'runmodule.php?module=worldmapen&op=continue&'))
            {
    ...
    elseif ('gypsy' == $op)
    {
        ...
        if ('' == $buymap)
        {
            ...
            addnav(['Buy World Map `0(`^%s gold`0)', $worldmapCostGold],
    ...
    if ($battle)
    {
        ...
        if ($victory)
        {
            ...
            else
            {
                ...
                {
                    //is talking
                    battlevictory($badguy, ($options['denyflawless'] ?? $battleDenyFlawless), $battleInForest);
                }
            }
