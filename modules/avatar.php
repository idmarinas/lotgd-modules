...
function avatar_dohook($hookname, $args)
{
    ...
    case 'lodge':
        ...
        {
            addnav(['Bio Avatar Picture (%s %s)', $cost,
                translate_inline(1 == $cost ? 'point' : 'points')],
                "runmodule.php?module=avatar&op=purchase&cost=$cost");
        }
        else
        {
            addnav(['Change Avatar Picture (%s %s)', $changecost,
                translate_inline(1 == $changecost ? 'point' : 'points')],
...
function avatar_run()
{
    ...
    case 'purchase':
        ...
        if ($pointsavail < $cost)
        {
            if (! get_module_pref('bought'))
            {
                output('`7He glances at hs ledger, "`&Unfortunately, purchasing a portrait will cost you %s points, and you currently only have %s %s available to spend.`7"', $cost, $pointsavail, translate_inline(1 == $pointsavail ? 'point' : 'points'));
            }
            else
            {
                output('`7He glances at hs ledger, "`&Unfortunately, changing your portrait will cost you %s points, and you currently only have %s %s available to spend.`7"', $cost, $pointsavail, translate_inline(1 == $pointsavail ? 'point' : 'points'));
            }
        }
        else
        {
