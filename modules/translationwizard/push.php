...
case "push":
	...
	$sql="SELECT intext,outtext,author,version FROM ".DB::prefix("translations")." where uri='".$namespace."' AND language='".$selectedlanguage."' ORDER BY intext;";
	$result=DB::query($sql);
	output("Please copy the following code to a file named `b`^%s.sql`0`b and give it to the admin for your language:",$namespace);
	output_notl("`n`n");
	$sql = "SELECT uri,count(*) AS c FROM " . DB::prefix("translations") . " WHERE language='".$selectedlanguage."' GROUP BY uri ORDER BY uri ASC";
	$res=DB::query($sql);
	...
	case "pushall": //get a grip :D
		...
		while (list($key,$rowspace) = each ($pusharray))
			{
			$end="'),".chr(13).chr(10);
			$sql="SELECT intext,outtext,author,version FROM ".DB::prefix("translations")." where uri='".$rowspace."' AND language='".$selectedlanguage."' ORDER BY intext;";
			$result=DB::query($sql);
			$i=0;
			$numend=DB::num_rows($result);
...
default:
	...
	$sql = "SELECT language,count(*) AS c FROM " . DB::prefix("translations") . " GROUP BY language ORDER BY language ASC";
	$result=DB::query($sql);
	while ($row = DB::fetch_assoc($result))
		{
