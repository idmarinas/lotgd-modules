...
function modloc_uninstall() { return true; }
...
function modloc_dohook($hookname, $args)
{
	...
	switch($hookname)
    {
        case 'footer-modules':
            ...
            {
                addnav('Navigation');
                addnav('View Module Locations', 'runmodule.php?module=modloc&admin=true');
            }
        break;
        case 'superuser':
            if ($session['user']['superuser'] & SU_MANAGE_MODULES)
...
function modloc_run()
{
    ...
    {
		addnav(array("%s", $name), "runmodule.php?module=modloc&admin=true&loc=".$name);
		$locations[$name]=0;
	}
	...
	output_notl("<table class='ui centered collapsing very compact striped table'><thead><tr><th>`&`b%s`b`0</th><th>`@`b%s`b`0</th><th>`^`b%s`b`0</th><th>`#`b%s`b`0</th></thead></tr>",translate_inline("Module Name"),translate_inline("Location Question"),translate_inline("Current Location"),translate_inline("Settings Link"),true);
	$n=0;
		...
		if (isset($info['settings']) && count($info['settings'])>0)
        {
			foreach ($info['settings'] as $key=>$val)
            {
				if (isset($val)&&!empty($val)&&isset($key)&&!empty($key))
                {
					...
					if (!is_array($x[0])) { $type = explode(',', $x[0]); }
					...
					if ($type == "location")
                    {
						...
						if (isset($locations[$l]))
                        {
                            ...
                            {
								output_notl("<tr><td>`&".$info['name']."`0</td>",true);
								output_notl("<td>`@".preg_replace('/,location/', '', $x[0])."`0</td>",true);
	...
	if ($n==0)
    {
		output_notl("<tr><td colspan=4>`c`b`@%s`b`c`0</td></tr>",translate_inline("No modules are hooked to this location."),true);
	}
	rawoutput("</table>");
	output("`n`@There are `^%s`@ module location settings.`0",$n);

	foreach ($locations as $name => $amount) { output("`n`^%s`@ has `^%s`@ modules hooked to it.",$name,$amount); }
	if ($error!=0) { output("`n`%There are `^%s`% modules hooked to nonexistent locations.",$error); }
