<?php
    ...
    if ('travel' == $op)
    {
        ...
        if ('' == $city)
        {
            ...
            if (! ($session['user']['superuser'] & SU_EDIT_USERS) && ($session['user']['turns'] <= 0) && 0 == $free)
            {
        ...
        else
        {
            ...
            if (e_rand(0, 100) < $dlevel && '1' != $su)
            {
                ...
                if (0 != module_events('travel', get_module_setting('travelspecialchance'), 'runmodule.php?module=cities&city='.urlencode($city)."&d=$danger&continue=1&"))
                {
    ...
    elseif ('fight' == $op || 'run' == $op)
    {
        if ('run' == $op && e_rand(1, 5) < 3)
        {
            ...
            if ($coward)
            {
                ...
                [
                    'foresttext' => ['In your terror, you lose your way and become lost, losing time for a forest fight.`n`n', $session['user']['location']],
                    'traveltext' => ['In your terror, you lose your way and become lost, losing precious travel time.`n`n', $session['user']['location']],
                ]
                );
            }
            output('After running for what seems like hours, you finally arrive back at %s.', $session['user']['location']);

            addnav(['Enter %s', $session['user']['location']], 'village.php');
            page_footer();
        }
        $battle = true;
    }
