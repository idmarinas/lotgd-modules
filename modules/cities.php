...
function cities_dohook($hookname, $args)
{
    ...
    $city = getsetting('villagename', LOCATION_FIELDS);
    $session['user']['location'] = $session['user']['location'] ?? '0';
    ...
    case 'master-autochallenge':
        ...
        if (get_module_pref('homecity') != $session['user']['location'])
        {
            $info = modulehook('cities-usetravel',
                [
                    'foresttext' => ['`n`n`^Startled to find your master in %s`^, your heart skips a beat, costing a forest fight from shock.', $session['user']['location']],
                    'traveltext' => ['`n`n`%Surprised at finding your master in %s`%, you feel a little less inclined to be gallivanting around the countryside today.', $session['user']['location']],
                ]
                );
    ...
    case 'villagetext':
        if ($session['user']['location'] == $city)
        {
            ...
            $args['title'] = ['%s, the Capital City', $city];
    ...
    case 'travel':
        ...
        $free = max(0, $args['available'] - $args['used']);
        addnav('Travelpoints');
        $p = translate_inline(1 != $free ? 'points' : 'point');
        $t = translate_inline(1 != $session['user']['turns'] ? 'turns' : 'turns');
        addnav(['You have `$%s %s free`0!', $free, $p], '');
        addnav(['You have `$%s %s free`0!', $session['user']['turns'], $t], '');
        ...
        if ($session['user']['location'] != $city)
        {
            addnav(['%s?Go to %s', $hotkey, $city], 'runmodule.php?module=cities&op=travel&city='.urlencode($city));
        }
        ...
        {
            addnav('Superuser');
            addnav(['%s?Go to %s', $hotkey, $city], 'runmodule.php?module=cities&op=travel&city='.urlencode($city).'&su=1');
        }
        break;
    case 'stablelocs':
