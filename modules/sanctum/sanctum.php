...
if ($op=="memberlist") {
	...
	$sql = "SELECT COUNT(*) AS c FROM " . DB::prefix("module_userprefs") . " WHERE modulename = 'sanctum' AND setting = 'member' AND value > 0";
	$result = DB::query($sql);
	...
	$sql = "SELECT ".DB::prefix("module_userprefs").".value, ".DB::prefix("accounts").".name FROM " . DB::prefix("module_userprefs") . "," . DB::prefix("accounts") . " WHERE acctid = userid AND modulename = 'sanctum' AND setting = 'member' AND value > 0 ORDER BY (0-value) DESC $limit";
	$result = DB::query($sql);
