...
function costumeshop_changeweapon($newweapon)
{
    ...
    $upgraded = false !== strpos($oldweapon, ' +1') ? true : false;
    $downgraded = false !== strpos($oldweapon, ' -1') ? true : false;
    $changed = false !== strpos($oldweapon, $newweapon) ? false : true;
...
function costumeshop_changearmor($newarmor)
{
    ...
    $upgraded = false !== strpos($oldarmor, ' +1') ? true : false;
    $downgraded = false !== strpos($oldarmor, ' -1') ? true : false;
    $changed = false !== strpos($oldarmor, $newarmor) ? false : true;
...
function costumeshop_dohook($hookname, $args)
{
    ...
    case 'footer-weapons':
        ...
        if ($weapon && $session['user']['weapon'] != $weapon && 'buy' == $op)
        {
    ...
    case 'footer-armor':
        ...
        if ($armor && $session['user']['armor'] != $armor && 'buy' == $op)
        {
    ...
    case 'newday':
        ...
        elseif ($rleft >= 2 && 'true' != $args['resurrection'])
        {
            $rleft--;
            set_module_pref('rentleft', $rleft);
        }
        elseif (1 == $rleft && 'true' != $args['resurrection'])
        {
            $rleft = 0;
            set_module_pref('rentleft', $rleft);
    ...
    case 'changesetting':
        ...
        $iscostume = false !== strpos($args['setting'], 'costume') ? true : false;

        if ($iscostume && 'costumeshop' == $args['module'])
        {
            $changed = false;
            ...
            if ($changed)
            {
                output('`7%s was truncated to %s`7 due to being too long.`n', $args['setting'], $changed);
            }
        }
        break;
    case 'dragonkilltext':
...
function costumeshop_run()
{
    ...
    else
    {
        ...
        for ($i = 1; $i <= 5; $i++)
        {
            ...
            if ('' != $costume)
            {
                addnav(['%s`0 costume (`^%s`0 gold)', $costume, $totalcost], 'runmodule.php?module=costumeshop&op=rent&costume='.$i);
                $n++;
            }
        }
        ...
        if (0 == $n)
        {
            addnav(['`$C`&l`@o`&w`$n`0 costume (`^%s`0 gold)', $totalcost], 'runmodule.php?module=costumeshop&op=rent&costume=clown');
            $n++;
        }
        ...
        if ($session['user']['donation'] > 0)
        {
            ...
            if ('' != $costume)
            {
                addnav(['%s`0 costume (`^%s`0 gold)', $costume, $totalcost], 'runmodule.php?module=costumeshop&op=rent&costume=lodge');
