...
function clantrees_dohook($hookname, $args)
{
    ...
    case 'village':
        if (get_module_setting('treebuy'))
        {
            ...
            if ($besttree)
            {
                $sql = 'SELECT clanname FROM '.DB::prefix('clans')." WHERE clanid='$besttree'";
                $res = DB::query($sql);
        ...
        if ('' == $op && (! $detail || ($detail == $clanid)))
        {
            ...
            if ($clanid && $session['user']['clanrank'] > CLAN_APPLICANT)
            {
                ...
                if (! $hastree && $treebuy)
                {
                    ...
                    if (CLAN_LEADER == $session['user']['clanrank'])
                    {
...
function clantrees_run()
{
    ...
    elseif ('treetime' == $op)
    {
        ...
        if (is_module_active('staminasystem'))
        {
            ...
            output('How many time do you wish to spend on the tree?');
            $stamina = get_stamina(3, true);
            $block = min(floor($stamina / 25000), 10);
            $turns = translate_inline('Spend stamina');
            $replyinfo = [
                'replystuff' => 'Time to spend (each time is 1.25% of stamina),range,0,'.$block.',1',
            ];
        }
        else
        {
    ...
    elseif ('alter' == $op)
    {
        ...
        if (0 == $howmuch)
        {
            output('`&You decide not to spend any %s on the tree right now.', translate_inline($what));
        }
