...
function advertising_getbanners($limited = false)
{
    ...
    if ($limited)
    {
        while (list($key, $val) = each($banners))
        {
            if (get_module_setting('banner_'.$key))
            {
                continue;
            }
            unset($banners[$key]);
...
function advertising_getallads()
{
    ...
    foreach ($result as $row)
    {
        ...
        {
            $types[$position]['banners'] = [$row];
            $types[$position]['priority'] = $row['priority'];
        }
        elseif ($types[$position]['priority'] == $row['priority'])
        {
            if (! isset($types[$position]['banners']))
            {
                $types[$position]['banners'] = [];
            }
    ...
    {
        if ($totalbanners >= get_module_setting('totalads'))
        {
            break;
        }
...
function advertising_sorttypes($a, $b)
{
    ...
    { //we don't want one type of banner to always override another type at the same priority level.
...
function advertising_install()
{
    ...
    $bannertable = [
        'bannerid' => ['type' => 'int(11) unsigned', 'extra' => 'auto_increment'],
        'owner' => ['type' => 'int(11) unsigned', 'default' => '0'],
        'size' => ['type' => 'varchar(20)'],
        'start' => ['type' => 'date', 'default' => '0000-00-00'],
        'end' => ['type' => 'date', 'default' => '2038-12-31'],
        'maximpressions' => ['type' => 'int(11) unsigned', 'default' => '4294967295'],
        'servedimpressions' => ['type' => 'int(11) unsigned', 'default' => '0'],
        'link' => ['type' => 'text'],
        'image' => ['type' => 'varchar(255)'],
        'priority' => ['type' => 'tinyint(4)', 'default' => '0'],
        'clicks' => ['type' => 'int(11) unsigned', 'default' => '0'],
        'approved' => ['type' => 'tinyint(4)', 'default' => ''.MOD_ADVERTISING_UNAPPROVED],
        'alttext' => ['type' => 'varchar(255)'],

        'key-PRIMARY' => ['type' => 'primary key', 'columns' => 'bannerid'],
        'key-daterange' => ['type' => 'key', 'columns' => 'start,end'],
        'key-impressions' => ['type' => 'key', 'columns' => 'servedimpressions'],
    ];
    //you can do these tables as $name=>array(descriptor) or array("name"=>$name,{descriptor})
    $adlog = [
        'logid' => ['name' => 'logid', 'type' => 'int(11) unsigned', 'extra' => 'auto_increment'],
        'bannerid' => ['name' => 'bannerid', 'type' => 'int(11) unsigned', 'default' => '0'],
        'userid' => ['name' => 'userid', 'type' => 'int(11) unsigned', 'default' => '0'],
        'ip' => ['name' => 'ip', 'type' => 'varchar(40)', 'default' => ''],
        'id' => ['name' => 'id', 'type' => 'varchar(32)', 'default' => ''],
        'date' => ['name' => 'date', 'type' => 'datetime', 'default' => '0000-00-00 00:00:00'],

        'key-PRIMARY' => ['name' => 'PRIMARY', 'type' => 'primary key', 'columns' => 'logid'],
        'key-date' => ['name' => 'date', 'type' => 'key', 'columns' => 'date'],
        'key-bannerid' => ['name' => 'bannerid', 'type' => 'key', 'columns' => 'bannerid'],
    ];
...
function advertising_dohook($hookname, $args)
{
    ...
    if ('everyfooter' == $hookname)
    {
        ...
        if ('installer' == $args['__scriptfile__'])
        {
            return $args;
        }
        global $session;
        ...
        if ($crawl > '')
        {
            ...
            {
                if ((1 == e_rand(1, 20) || ! $fe) && isset($session['user']['output']) && false === strpos($session['user']['output'], '<!--Su_Restricted-->'))
                {
    ...
    elseif ('superuser' == $hookname)
    {
        if ($session['user']['superuser'] & MOD_ADVERTISING_SUAPPROVER)
        {
            ...
            $row = $result->current();
            addnav('Mechanics');
            addnav(['Ad Banner Approval (%s)', $row['c']], 'runmodule.php?module=advertising&op=bannerapprover', false, true);
        }
    }
...
function advertising_run()
{
    ...
    elseif ('upload' == $op)
    {
        ...
        rawoutput('<ul>');
        $places = ['headerad' => 'page header', 'navad' => 'navigational area', 'bodyad' => 'page body', 'verticalad' => 'right-aligned in page body'];
        reset($banner_list);
    ...
    elseif ('bannerapprover' == $op)
    {
        ...
        foreach ($result as $row)
        {
            ...
            rawoutput('</td><td>');
            output(
                MOD_ADVERTISING_APPROVED == $row['approved'] ? '`^Approved`0' :
                    (MOD_ADVERTISING_DENIED == $row['approved'] ? '`$Denied`0' :
                        '`#UnApproved`0'
                    )
                );
            rawoutput('</td><td>');
    ...
    elseif ('getbanner' == $op)
    {
        ...
        else
        {
            ...
            {
                $genericbanners = modulehook('mod_ad_banner', ['size' => $type, 'w' => $w, 'h' => $h, 'banners' => []]);
                //remove keys if any
...
function advertising_checkcrawler()
{
    ...
    $bots = ['bot', 'slurp', 'grub', 'crawl', 'scan', 'szukacz', 'mozilla'];
