...
function alt_char_list_run()
{
    ...
    switch ($op)
    {
		case "":
            ...
            if (($pageoffset + $pp) < $total) { $cond = $pageoffset + $pp; }
            else { $cond = $total; }
            ...
            if (DB::num_rows($result)>0)
            {
				...
				foreach ($result as $row)
				{
                    ...
                    if (DB::num_rows($res_ip) > 0)
                    {
                        while ($row_ip = DB::fetch_assoc($res_ip))
                        {
                    ...
                    if (DB::num_rows($res_id) > 0)
                    {
                        while ($row_id = DB::fetch_assoc($res_id))
                        {
                    ...
                    if (DB::num_rows($res_email) > 0)
                    {
                        while ($row_email = DB::fetch_assoc($res_email))
                        {
