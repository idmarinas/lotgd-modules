...
function drinks_drunkenize($commentary, $level)
{
    ...
    while ($replacements / strlen($straight) < ($level) / 500)
    {
        ...
        if (mt_rand(0, 9))
        {
            ...
            if (false !== $x &&
                '*hic*' != substr($commentary, $x, 5) &&
                '*hic*' != substr($commentary, max($x - 1, 0), 5) &&
                '*hic*' != substr($commentary, max($x - 2, 0), 5) &&
                '*hic*' != substr($commentary, max($x - 3, 0), 5) &&
                '*hic*' != substr($commentary, max($x - 4, 0), 5))
            {
