...
*/
    ...
    if (! empty($allprefs))
    {
        ...
        for ($i = 0; $i < $count; $i++)
        {
            if (0 == $keep_days || ($allprefs['date'][$i] + $seconds) > time())
            {
                ...
                $trans_reasons = (999 == $reasons[$allprefs['reason'][$i]]) ? translate_inline('Unknown') : translate_inline($reasons[$allprefs['reason'][$i]]);
    ...
    if ($rows_returned <= 0)
    {
        ...
        output('`#%s `3has `$%s %s `3so far. %s more for a %s day ban.', $row_name, $count, translate_inline(1 == $count ? 'warning' : 'warnings'), $warns_left, $ban_days);
        rawoutput('</td></tr>');

        if (1 == $warns_left)
        {
            rawoutput('<tr class="trdark"><td colspan="2">');
            output('`^If you warn this person once more, they will be banned for %s %s. If you are sure then please proceed.', $ban_days, translate_inline(1 == $ban_days ? 'day' : 'days'));
            rawoutput('</td></tr>');
        }
