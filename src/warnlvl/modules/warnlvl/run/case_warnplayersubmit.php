...
*/
    ...
    if ($warns_count >= $warns_total)
    {
        ...
        $days = translate_inline(1 == $ban_days ? 'day' : 'days');

        output('`#%s `3has been banned for %s %s. They `iwere`i warned.', $username, $ban_days, $days);

        addnews('`#%s `3has been `$banned `3for `^%s %s`3. %s should have taken the %s as a sign.', $username, $ban_days, $days, translate_inline(1 == $row['sex'] ? 'She' : 'He'), translate_inline(1 == $warns_count ? 'warning' : 'warnings'), true);
        ...
        $mailmessage = translate_mail([$msg, $username, get_suffix($warns_count), $ban_days, $days, $reasons[$reason], $yom_text]);
    ...
    else
    {
        ...
        // Warned.
        //
        output('`#%s `3has been sent a YoM, this was %s `$%s warning`3. %s in total.', $username, translate_inline(1 == $row['sex'] ? 'her' : 'his'), get_suffix($warns_count), $allprefs['warnings']);
        ...
        $mailmessage = translate_mail([$msg, $username, get_suffix($warns_count), $reasons[$reason], $yom_text]);
        systemmail($id, $subject, $mailmessage);
    }
