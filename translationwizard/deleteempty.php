...
switch ($mode)
	...
	case "del": //single delete action
		...
		$sql = "DELETE FROM " . DB::prefix("untranslated") . " WHERE intext = '$intext' AND namespace='' AND language='$language'";
		if ($intext<>'') DB::query($sql);
	...
	case "listing":  //if the user hits the button to work on a list, one by one
		$sql= "SELECT intext, language FROM  ".DB::prefix("untranslated")." WHERE namespace='' GROUP  BY BINARY intext, language";
		$result = DB::query($sql);
		output("`n`n %s rows have been found with no namespace in your untranslated table.`n`n",DB::num_rows($result));
		$i = 0;
