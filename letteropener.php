...
function letteropener_run(){
    ...
    if ($op=="read")
    {
        ...
        $sqlz = "SELECT login from " . DB::prefix("accounts") . " where acctid=\"".$acctid."\"";
        ...
        $sql = "SELECT " . DB::prefix($maildb) . ".*,". DB::prefix("accounts"). ".name,login FROM " . DB::prefix($maildb) ." LEFT JOIN " . DB::prefix("accounts") . " ON ". DB::prefix("accounts") . ".acctid=" . DB::prefix($maildb). ".msgfrom WHERE msgto=\"".$acctid."\" AND messageid=\"".$id."\"";
        $result = DB::query($sql);
        if ($result->count() > 0)
        {
            ...
            if ((int)$row['msgfrom']!=0){
				...
				//$sql="select login from ".DB::prefix("accounts")." where acctid=$othername";
				//$result = DB::query($sql);
				$othername=$row['login'];
            ...
            rawoutput("</tr><tr>");
            $sql = "SELECT messageid FROM ".DB::prefix($maildb)." WHERE msgto='{$acctid}' AND messageid < '$id' ORDER BY messageid DESC LIMIT 1";
            $result = DB::query($sql);
            ...
            $sql = "SELECT messageid FROM ".DB::prefix($maildb)." WHERE msgto='{$acctid}' AND messageid > '$id' ORDER BY messageid  LIMIT 1";
            $result = DB::query($sql);
            if (DB::num_rows($result)>0){
                $row = DB::fetch_assoc($result);
                $nid = $row['messageid'];
        ...
        $sql="select * from ".DB::prefix($maildb)." where msgfrom>0 ORDER BY messageid DESC limit ".get_module_setting("num")."";
        $res=DB::query($sql);
        ...
        for ($i=0;$i<DB::num_rows($res);$i++){
            ...
            $sql2="select name from ".DB::prefix("accounts")." where acctid=".$row['msgto']."";
            $res2=DB::query($sql2);
            $row2=DB::fetch_assoc($res2);
            $toname=color_sanitize($row2['name']);
            $sql3="select name from ".DB::prefix("accounts")." where acctid=".$row['msgfrom']."";
            $res3=DB::query($sql3);
            $row3=DB::fetch_assoc($res3);
        ...
        $where = "";
        $op="";
        $sql = "SELECT acctid,login,name FROM " . DB::prefix("accounts");
        if ($query != "") {
            $where = "WHERE login='$query' OR name='$query'";
            $searchresult = DB::query($sql . " $where  ORDER BY '$order' LIMIT 2");
        }
        ...
        if ($query !== false || $searchresult) {
            if (DB::num_rows($searchresult) != 1) {
                ...
                $searchresult = DB::query($sql . " $where  ORDER BY '$order' LIMIT 101");
            }
        ...
        $from = httpget('from');
        if ($to!=""){
            $sql = "SELECT acctid,login,name superuser FROM " . DB::prefix("accounts") . " WHERE login=\"$to\"";
            $result = DB::query($sql);
            $row = DB::fetch_assoc($result);
            $sql = "SELECT acctid FROM " . DB::prefix("accounts") . " WHERE login='".$row['login']."'";
            $result = DB::query($sql);
            $row2 = DB::fetch_assoc($result);
            $acctid=$row2['acctid'];
            rawoutput("<table>");
            $session['message']="";
            $sql = "SELECT subject,messageid," . DB::prefix("accounts") . ".name,msgfrom,seen,sent FROM " . DB::prefix($maildb) . " LEFT JOIN " . DB::prefix("accounts") . " ON " . DB::prefix("accounts") . ".acctid=" . DB::prefix($maildb) . ".msgfrom WHERE msgto=\"".$acctid."\" ORDER BY sent DESC";
            $result = DB::query($sql);
            if ($result->count() > 0)
            {
        ...
        $from = httpget('from');
        if ($from!=""){
            $sql = "SELECT acctid,login,name superuser FROM " . DB::prefix("accounts") . " WHERE login=\"$from\"";
            $result = DB::query($sql);
            $row = DB::fetch_assoc($result);
            $sql = "SELECT acctid FROM " . DB::prefix("accounts") . " WHERE login='".$row['login']."'";
            $result = DB::query($sql);
            ...
            output_notl("<table>",true);
            $session['message']="";
            $sql = "SELECT subject,messageid," . DB::prefix("accounts") . ".name,msgto,seen,sent FROM " . DB::prefix($maildb) . " LEFT JOIN " . DB::prefix("accounts") . " ON " . DB::prefix("accounts") . ".acctid=" . DB::prefix($maildb) . ".msgto WHERE msgfrom=\"".$acctid."\" ORDER BY sent DESC";
            $result = DB::query($sql);
            if (DB::num_rows($result)>0){
                foreach ($result as $row)
                {
